...
public class FpsDisplay : MonoBehaviour
{
    ...
    private void Update()
    {
        Display(_averageLabel, "avg ", _fpsCounter.AverageFPS);
        Display(_highestLabel, "max ", _fpsCounter.HighestPFS);
        Display(_lowestLabel, "min ", _fpsCounter.LowersFPS);
    }

    private void Display(Text label, string type, int fps)
    {
        label.text = type + _stringsFrom00To99[Mathf.Clamp(fps, 0, 99)];
